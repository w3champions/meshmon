//! Server-Sent Events handler for the catalogue broker.
//!
//! A single `GET /api/catalogue/stream` connection per client. The
//! handler subscribes to the broker on [`crate::state::AppState`] and
//! forwards every [`super::events::CatalogueEvent`] as an `Event::data`
//! frame with the JSON-serialized payload. A 15-second keep-alive
//! comment keeps intermediate proxies from idling the connection out.
//!
...
#[utoipa::path(
    get,
    path = "/api/catalogue/stream",
    tag = "catalogue",
    responses(
        (status = 200, description = "SSE stream of catalogue changes"),
    ...
    let stream = BroadcastStream::new(rx).map(|res| {
        let payload = match res {
            Ok(ev) => serde_json::to_string(&ev).expect(
                "CatalogueEvent serialization is infallible — all fields are Uuid/String/enum",
            ),
            Err(err) => {
